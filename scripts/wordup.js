...
function addNewWordSubmission(word) {
    ...
    // done 21
    // checks if submitted word has been pushed before does not matter if word is real or not.
    var alreadyUsed = model.wordSubmissions.filter(aWord =>{
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        // give the scoreChip appropriate text content and css classes
...
function containsOnlyAllowedLetters(word) {
    ...
    var itIsGood = disallowedLettersInWord(word)

    if (itIsGood.length > 0)
        {return false;}
    else
        {return true;}
}
